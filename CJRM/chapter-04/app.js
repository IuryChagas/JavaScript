...
for (let x = 1; x <= 10; x++) {
    result(`Esta é a ${x}ª vez que esta frase é exibida no console.`)
}
...
const cities = ['São Paulo', 'Paraná', 'Minas Gerais']
...
const templateHTML = `<li>item</li>`

for (let i = 0; i < names.length; i++) {
    const name = names[i]

    result(templateHTML.replace('item', `${name}`))
...
for(let arrayNumbers = 0; arrayNumbers < arrays.length; arrayNumbers++){
    for(let number = 0; number < arrays[arrayNumbers].length; number++){
        sumTotal += arrays[arrayNumbers][number]
    }
}

/*
  01

  - Faça com que o texto do h1 do index.html seja exibido com todas as letras  
    maiúsculas.
*/
console.info('\n### Etapa 05 - Exercício: 01 ###\n ')

const title = document.querySelector('h1')

const uperCaseTitle = title.textContent.toUpperCase()
console.log(uperCaseTitle)

/*
  02

  - Adicione, na ul vazia do index.html, os números do array "numbers" abaixo;
  - Cada número deve estar dentro de uma <li> com a classe "number".
*/
console.info('\n### Etapa 05 - Exercício: 02 ###\n ')
const numbers = [ 53, 24, 3, 8, 1, 6, 57, 80, 77, 98, 55 ]

let ul = document.querySelector('ul')

numbers.forEach(num => {
  ul.innerHTML += `<li class='number'>${num}</li>`
})

ul.classList.add('strong')
/*
  03

  - Modifique as cores dos números dentro da ul da seguinte forma:
    - Se o número é par, ele deve ser exibido na cor "lightblue";
    - Se o número é ímpar, exiba-o na cor "pink".
*/
console.info('\n### Etapa 05 - Exercício: 03 ###\n ')

let list = document.querySelectorAll('li')

const changeColor = numb => {

  let evenNumber = +numb.textContent % 2 === 0
  let oddNumber = +numb.textContent % 2 !== 0

    if(evenNumber){
      numb.classList.add('lightblue')
    }
    if(oddNumber){
      numb.classList.add('pink')
    }
  console.log(numb)
}

list.forEach(changeColor)

/*
  04

  - Adicione a classe "body-background" no corpo do index.html;

  P.s: a classe "body-background" já está declarada no style.css.
*/
console.info('\n### Etapa 05 - Exercício: 04 ###\n ')

// let indexHTML = document.querySelector('body')

let body = document.body
body.classList.add('body-background')

console.log(body.getAttribute('class'))

/*
  05

  - Adicione o link 
    https://github.com/roger-melo-treinamentos/curso-de-js-roger-melo no href  
    do link do index.html.
*/
console.info('\n### Etapa 05 - Exercício: 05 ###\n ')

let href = document.querySelector('a')

href.setAttribute('href','https://github.com/IuryChagas/JavaScript/tree/master/CJRM')

/*
  06

  - Exiba o novo valor do atributo href do link no console.
*/
console.info('\n### Etapa 05 - Exercício: 06 ###\n ')

console.log(href.getAttribute('href'))

/*
  07

  - Exiba, no console, um objeto com todas as propriedades CSS que podem ser  
    manipuladas via JS no h1.
*/
console.info('\n### Etapa 05 - Exercício: 07 ###\n ')

let h1 = document.querySelector('h1')

console.log(h1.style)


/*
  08

  - Remova a classe "body-background", do elemento body.
*/
console.info('\n### Etapa 05 - Exercício: 08 ###\n ')

body.classList.remove('body-background')

/*
  09

  - Se o link da página possuir uma classe "link", remova-a;
  - Não utilize o método remove() para fazer isso.
*/
console.info('\n### Etapa 05 - Exercício: 09 ###\n ')

href.classList.toggle('link')
